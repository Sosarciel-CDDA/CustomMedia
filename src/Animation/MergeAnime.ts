...
import { PkgTilesetInfo, TilesetCfg, ModTileset } from "@sosarciel-cdda/schema";
...
export async function mergeAnime(dm:DataManager,charName:string,forcePackage:boolean=true){
    ...
    const animModTileset:ModTileset = {
        ...
        "tiles-new": tilesetNew.map(item=>{
            item.file = path.join(getOutAnimPath(charName),item.file)
            return item;
        }),
    }
    //dm.addData([animModTileset,ordering], path.join(getOutAnimPath(charName),"anime_tileset"))
    dm.addData([animModTileset], path.join(getOutAnimPath(charName),"anime_tileset"))
